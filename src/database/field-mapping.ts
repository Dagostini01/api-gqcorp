...
function parseDecimal(value: string | number): number | undefined {
  ...
  if (lastDot !== -1 && lastComma !== -1) {
    ...
    normalized = cleaned.replace(reThousands, '');
    // Normaliza decimal para ponto
    if (decimalSep === ',') normalized = normalized.replace(/,/g, '.');
  } else if (lastComma !== -1) {
    // Apenas vírgula: considerar vírgula como decimal
    // Se houver múltiplas vírgulas, remover todas exceto a última
    normalized = cleaned.replace(/,(?=.*,)/g, '').replace(/,/g, '.');
  } else if (lastDot !== -1) {
    // Apenas ponto: se houver múltiplos pontos, remover todos exceto o último
